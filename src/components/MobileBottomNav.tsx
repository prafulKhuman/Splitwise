...
export default function MobileBottomNav() {
  ...
  return (
    <Paper elevation={8} sx={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 1200, borderRadius: 0, pb: "env(safe-area-inset-bottom)" }}>
      <BottomNavigation value={current} onChange={(_, idx) => router.push(navItems[idx].href)} showLabels
        sx={{
          height: 64,
          "& .MuiBottomNavigationAction-root": { minWidth: 0, py: 1, color: "#B5B5C3", "&.Mui-selected": { color: "#6C63FF" } },
          "& .MuiBottomNavigationAction-label": { fontSize: "0.6rem", fontWeight: 600, "&.Mui-selected": { fontSize: "0.6rem" } },
        }}>
